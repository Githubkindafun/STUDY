...
int boardEvaluation(Board& board) {
    ...
    // podrabane ze stacka (idea)


    const pair<int, int> SQUARE_X[4] = {
        {1, 1}, {1, 6}, {6, 1}, {6, 6} 
    };
    ...
    for(int i = 0; i < 4; i++) {
        int row = CORNERS[i].first;
        int col = CORNERS[i].second;
        int xrow = SQUARE_X[i].first;
        int xcol = SQUARE_X[i].second;
        if(board.grid[row][col] == EMPTY) {
            if(board.grid[xrow][xcol] == BLACK) {
                squareXAmount++;
            } else if(board.grid[xrow][xcol] == WHITE) {
                squareXAmount--;
            }
        }
    }

    const pair<int, int> SQUARE_C[4][2] = {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        if(board.grid[row][col] == EMPTY) {
            for(int j = 0; j < 2; j++) {
                int crow = SQUARE_C[i][j].first;
                int ccol = SQUARE_C[i][j].second;
                if(board.grid[crow][ccol] == BLACK) {
                    squareCAmount++;
                } else if(board.grid[crow][ccol] == WHITE) {
                    squareCAmount--;
                }
            }
        }
    ...
    int finalEvaluation = 30 * discDiffrence + 500 * cornersTaken + 150 * diffStableDiscs 
    + (-300) * squareXAmount + (-100) * squareCAmount;
...
int main() {
    ...
    while(cin >> msg) {
        if(msg == "UGO") {
            ...
            // liczymy
            pair<int, int> bestMove = selectBestMove(board, 8);
